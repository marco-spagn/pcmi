...
### `make test-streams-integration`Runs `go test -tags=integration -run TestStream ./internal/event/...`with **miniredis** in-process. No Postgres, Redis, or `:50051`required. Optional before PRs that touch `internal/event`Streams backend; not part of `act-integration-smoke`today.
...
```bash
make test-full-real
```Equivalent: `./scripts/run_full_validation.sh`### Prerequisites
