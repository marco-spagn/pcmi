...
COVERAGE_ENDPOINT_OUT="${COVERAGE_ENDPOINT_OUT:-}"if [ ! -f "$COVERAGE_OUT"]; then
...
NR == 1 { next } # skip the "mode: <m>"header
{
  ...
  pkg = "(other)"if (match(fp, /\/internal\/[^\/]+\//)) {
...
' "$COVERAGE_OUT")"if [ -z "$SUMMARY"]; then
...
report_text() {
  printf "%-20s %10s %10s %10s\n""PACKAGE""STMTS""COVERED""COVERAGE"printf "%-20s %10s %10s %10s\n""-------""-----""-------""--------"printf '%s\n' "$SUMMARY"| awk -F'\t' '$1=="PKG"{
    pct = ($3 > 0) ? ($4/$3)*100 : 0
...
REPORT="$(report_text)"
printf '%s\n' "$REPORT"if [ -n "$COVERAGE_SUMMARY_OUT"]; then
...
if [ -n "$COVERAGE_MD_OUT"]; then
  {
    echo "## ${GLOBAL_EMOJI} Test coverage: \`${GLOBAL_PCT}%\`"echo ""echo "_Global statement coverage; threshold = **${COVERAGE_MIN_TOTAL}%** (\`scripts/ci_coverage_check.sh\`)._"echo ""echo "| Package | Stmts | Covered | Coverage |"echo "|---|---:|---:|---:|"printf '%s\n' "$SUMMARY"| awk -F'\t' -v floors="$COVERAGE_PKG_FLOORS"-v gmin="$COVERAGE_MIN_TOTAL"'
      BEGIN {
        n = split(floors, a, ",")
        for (i = 1; i <= n; i++) {
          if (split(a[i], kv, ":") == 2) {
      ...
      $1=="PKG"{
        ...
        pct = ($3 > 0) ? ($4/$3)*100 : 0
        if (pkg in floor) {
          emoji = (pct >= floor[pkg]) ? "": ""} else {
          min = gmin + 0
          if (pct >= min) emoji = ""else if (pct >= 30) emoji = ""else emoji = ""}
        printf "| `%s`| %d | %d | %s %.1f%% |\n", pkg, $3, $4, emoji, pct
      }
    ' | sort
    echo "| **Global** | $(printf '%s\n' "$SUMMARY"| awk -F'\t' '$1=="GLOBAL"{printf "**%d** | **%d** | **%s %.1f%%**", $3, $4, "'"$GLOBAL_EMOJI"'", ($4/$3)*100}') |"echo ""echo "<sub>Generated by \`scripts/ci_coverage_check.sh\`— bump floors in the same PR that lifts coverage.</sub>"} > "$COVERAGE_MD_OUT"
fi
...
if [ -n "$COVERAGE_BADGE_OUT"]; then
  ...
  BADGE_COLOR="$(awk -v p="$GLOBAL_PCT"'BEGIN {
    p += 0
    if (p >= 90) print "brightgreen"else if (p >= 75) print "green"else if (p >= 60) print "yellowgreen"else if (p >= 40) print "yellow"else if (p >= 20) print "orange"else print "red"}')"# Round to 1 decimal for the badge label.
  BADGE_PCT="$(awk -v p="$GLOBAL_PCT"'BEGIN { printf "%.1f", p+0 }')"BADGE_URL="https://img.shields.io/badge/coverage-${BADGE_PCT}%25-${BADGE_COLOR}.svg"printf '%s\n' "$BADGE_URL"> "$COVERAGE_BADGE_OUT"
fi
...
if [ -n "$COVERAGE_ENDPOINT_OUT"]; then
  ENDPOINT_COLOR="$(awk -v p="$GLOBAL_PCT"'BEGIN {
    p += 0
    if (p >= 90) print "brightgreen"else if (p >= 75) print "green"else if (p >= 60) print "yellowgreen"else if (p >= 40) print "yellow"else if (p >= 20) print "orange"else print "red"}')"ENDPOINT_PCT="$(awk -v p="$GLOBAL_PCT"'BEGIN { printf "%.1f", p+0 }')"mkdir -p "$(dirname "$COVERAGE_ENDPOINT_OUT")"cat > "$COVERAGE_ENDPOINT_OUT"<<JSON
{
  "schemaVersion": 1,
  "label": "coverage",
...
if [ -n "$COVERAGE_PKG_FLOORS"]; then
  IFS=',' read -r -a floors <<< "$COVERAGE_PKG_FLOORS"for entry in "${floors[@]}"; do
    [ -z "$entry"] && continue
    pkg="${entry%%:*}"pct="${entry##*:}"if [ "$pkg"= "$entry"] || [ -z "$pkg"] || [ -z "$pct"]; then
      echo "::error::malformed COVERAGE_PKG_FLOORS entry: $entry">&2
      fail=1
      continue
    fi

    PKG_PCT="$(printf '%s\n' "$SUMMARY"| awk -F'\t' -v p="$pkg"'$1=="PKG"&& $2==p { if ($3>0) printf "%.2f", ($4/$3)*100; else print "0.00"}')"if [ -z "$PKG_PCT"]; then
      echo "::warning::package floor for '$pkg' configured but package not present in coverage profile"continue
    fi
    ...
    if ! ge "$PKG_PCT""$pct"; then
      ...
      echo "ok: package '$pkg' coverage ${PKG_PCT}% >= ${pct}%"fi
  done
fi
