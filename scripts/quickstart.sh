...
_spinner_msg=""
spinner_start() {
  _spinner_msg="$1"printf "${YELLOW}  %s${RESET}""$_spinner_msg"( i=0; frames='⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏'
    while true; do
      printf '\r'"${YELLOW}%s %s${RESET}""${frames:$((i % 10)):1}""$1"sleep 0.1
      i=$((i + 1))
    done
  ) &
  _spinner_pid=$!
...
spinner_stop() {
  if [ -n "$_spinner_pid"]; then
    ...
    _spinner_pid=""# clear the spinner line completely before the caller prints ok/err
    printf '\r\033[K'
  fi
}
...
info "Checking dependencies..."missing=()
...
if [ "${#missing[@]}"-gt 0 ]; then
  err "Missing required dependencies:"for m in "${missing[@]}"; do
    printf "• %s\n""$m">&2
  done
  exit 1
fi

ok "All dependencies present (docker, docker compose, curl, jq)"# ── Step 2: Copy .env.example ─────────────────────────────────────────────────
header "Environment"
if [ ! -f .env ]; then
  if [ ! -f .env.example ]; then
    err ".env.example not found — are you running from the repo root?"exit 1
  fi
  cp .env.example .env
...
if ! docker compose up -d --build --wait >/dev/null 2>&1; then
  spinner_stop
  err "docker compose up failed — check: docker compose logs"exit 1
fi
spinner_stop
ok "Stack is up"# ── Step 4: Wait for health ────────────────────────────────────────────────────
header "Health check"
...
while true; do
  ...
  if [ "$(date +%s)"-ge "$deadline"]; then
    spinner_stop
    err "API did not become healthy within ${HEALTH_TIMEOUT}s."err "Check logs: docker compose logs api"exit 1
  fi
  sleep 2
done
spinner_stop
ok "API is healthy at ${API_URL}"# ── Helper: store a memory ─────────────────────────────────────────────────────
store_memory() {
  local path="$1"content="$2"tags="$3"pcurl -f --max-time 10 -X POST "${API_URL}/v1/memories"\
    -d "{\"path\":\"${path}\",\"content\":\"${content}\",\"tags\":${tags}}"\
    >/dev/null
}

# ── Step 5: Store memories ─────────────────────────────────────────────────────
header "Storing memories"info "SOC scenario — 3 alerts under root.security.alerts"
...
ok "Stored: root.security.alerts.brute_force"store_memory "root.security.alerts.lateral_movement"\
...
MEMORIES_STORED=$((MEMORIES_STORED + 1))
ok "Stored: root.security.alerts.lateral_movement"store_memory "root.security.alerts.exfil_attempt"\
...
MEMORIES_STORED=$((MEMORIES_STORED + 1))
ok "Stored: root.security.alerts.exfil_attempt"info "Trading scenario — 2 signals under root.trading.signals"
...
MEMORIES_STORED=$((MEMORIES_STORED + 1))
ok "Stored: root.trading.signals.btc_breakout"store_memory "root.trading.signals.eth_oversold"\
...
MEMORIES_STORED=$((MEMORIES_STORED + 1))
ok "Stored: root.trading.signals.eth_oversold"info "DevOps scenario — 1 incident under root.devops.incidents"
...
MEMORIES_STORED=$((MEMORIES_STORED + 1))
ok "Stored: root.devops.incidents.db_cpu_spike"printf "\n  Total memories stored: ${GREEN}%d${RESET}\n""$MEMORIES_STORED"# ── Step 6: Retrieve memories ─────────────────────────────────────────────────
header "Retrieving memories"
info "Listing all memories under root.security..."MEMORIES_RETRIEVED=$(pcurl --max-time 10 -X POST "${API_URL}/v1/retrieve"\
...
ok "Retrieved ${MEMORIES_RETRIEVED} memories under root.security"# ── Step 7 & 8: Distillation ──────────────────────────────────────────────────
header "Distillation"
...
ok "Stored 2 seed memories under root.test for distillation"info "Polling /v1/distilled?path_prefix=root.test (max 60s, worker fires every ~15s)..."
...
if [ "$MEMORIES_DISTILLED"-gt 0 ]; then
  ok "Found ${MEMORIES_DISTILLED} distilled insight(s):"printf '%s' "$dist_result"\
    | jq -r '.entries[]? | "• \(.path): \(.content // .summary | .[0:100])…"' 2>/dev/null || true
else
  warn "No distilled results yet — worker needs OPENAI_API_KEY in .env to run distillation."warn "Set OPENAI_API_KEY=sk-... in .env, restart the stack, and re-run."
fi
...
printf "%-25s ${GREEN}%d${RESET}\n""Distilled insights""$MEMORIES_DISTILLED"# ── Step 10: Next steps ────────────────────────────────────────────────────────
header "Next steps"
cat <<EOF
  ${CYAN}Install the Python SDK:${RESET}
    pip install pcmi-sdk
    export PCMI_BASE_URL="${API_URL}"export PCMI_API_KEY="${API_KEY}"${CYAN}Install the TypeScript SDK:${RESET}
  ...
  ${CYAN}Admin dashboard (browser):${RESET}
    open "${API_URL}/v1/admin/ui?key=${API_KEY}"${CYAN}Read the docs:${RESET}
