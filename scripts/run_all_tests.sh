...
[ -n "$GT_P"] && GT_FLAGS="$GT_P $GT_FLAGS"# ── cleanup on exit ───────────────────────────────────────────────────────────
on_exit() {
  local code=$?
  if [ "$POSTGRES_STARTED_BY_US"-eq 1 ] && [ "$NO_CLEANUP"-eq 0 ]; then
    echo -e "\n${YELLOW}→ stopping Postgres started by this script...${RESET}"$DOCKER_COMPOSE stop postgres redis >/dev/null 2>&1 || true
  fi
  echo -e "\n${BOLD}══ SUMMARY ══${RESET}"echo -e "${GREEN} passed${RESET}:   $PASS"echo -e "${RED} failed${RESET}:   $FAIL"echo -e "${YELLOW}⊘ skipped${RESET}:  $SKIP"if [ "${#FAILED_ITEMS[@]}"-gt 0 ]; then
    echo -e "\n${RED}${BOLD}╔══ WHAT FAILED ══╗${RESET}"for item in "${FAILED_ITEMS[@]}"; do
      echo -e "${RED}${RESET}  $item"done
    echo -e "${RED}${BOLD}╚════════════════════╝${RESET}"echo -e "\n  Tip: re-run only the failures with:"echo -e "${CYAN}DATABASE_URL=... go test -race -count=1 -tags integration -v -run <TestName> ./internal/handler/...${RESET}"fi

  [ "$FAIL"-eq 0 ] && echo -e "\n  ${GREEN}${BOLD}ALL TESTS PASSED${RESET}"\
                     || echo -e "\n  ${RED}${BOLD}BUILD FAILED${RESET}"exit "$code"
}
trap on_exit EXIT
...
if [ "$INTEGRATION_ONLY"-eq 0 ]; then
  ...
  section "A1 — build e vet"if go build -o /dev/null ./...; then ok "go build"; else fail "go build"; fi
  if go vet ./...; then ok "go vet"; else fail "go vet"; fi

  section "A2 — unit test + stress test (-race, all packages)"echo "→ go test $GT_FLAGS ./..."# shellcheck disable=SC2086
  if go test $GT_FLAGS ./...; then
    ok "unit + stress test"else
    fail "unit + stress test — see output above"fi

  section "A3 — stress test individual packages (detailed report)"# Runs each package with stress tests in isolation for readable output.
  STRESS_PKGS=(
    "github.com/marco-spagn/pcmi/internal/event""github.com/marco-spagn/pcmi/internal/embedding""github.com/marco-spagn/pcmi/internal/webhook""github.com/marco-spagn/pcmi/internal/worker""github.com/marco-spagn/pcmi/internal/service""github.com/marco-spagn/pcmi/internal/handler")
  for pkg in "${STRESS_PKGS[@]}"; do
    name="${pkg##*/}"# shellcheck disable=SC2086
    if go test $GT_FLAGS -run "Stress""$pkg"2>&1 | tail -3 | grep -qE "^ok|no test files"; then
      ok "stress ${name}"else
      # Re-run without grep to show the error
      # shellcheck disable=SC2086
      go test $GT_FLAGS -run "Stress""$pkg"|| fail "stress ${name}"fi
  done
...
# ══════════════════════════════════════════════════════════════════════════════
section "B — infrastructure (Postgres)"pg_is_ready() {
...
if pg_is_ready; then
  ...
  echo "→ starting postgres + redis via docker compose..."$DOCKER_COMPOSE up -d postgres redis >/dev/null
  POSTGRES_STARTED_BY_US=1

  echo "→ waiting for Postgres to be ready (max 120s)..."OK=0
  for i in $(seq 1 120); do
    if pg_is_ready; then OK=1; break; fi
    sleep 1
    [ $((i % 10)) -eq 0 ] && echo "… ${i}s"done
  if [ "$OK"-eq 1 ]; then
    ok "Postgres ready"else
    fail "Postgres not responding after 120s"exit 1
  fi
fi
...
go test $GT_FLAGS -tags integration \
    ...
    -timeout 2m -v && ok "DataVisibility"|| fail "DataVisibility"section "D2 — multi-instance: concurrent writes from all replicas"
# shellcheck disable=SC2086
go test $GT_FLAGS -tags integration \
    ./internal/handler/... \
    -run "TestMultiInstance_ConcurrentWritesFromAllInstances"\
    -timeout 3m -v && ok "ConcurrentWrites (5 replicas × 100 goroutines)"|| fail "ConcurrentWrites"section "D3 — multi-instance: Redis event propagation"
# shellcheck disable=SC2086
go test $GT_FLAGS -tags integration \
    ./internal/handler/... \
    -run "TestMultiInstance_RedisEventPropagation"\
    -timeout 2m -v && ok "RedisEventPropagation"|| fail "RedisEventPropagation"section "D4 — multi-instance: concurrent writes to the same path"
# shellcheck disable=SC2086
go test $GT_FLAGS -tags integration \
    ./internal/handler/... \
    -run "TestMultiInstance_SamePathConcurrentWrites"\
    -timeout 2m -v && ok "SamePathConcurrentWrites"|| fail "SamePathConcurrentWrites"section "D5 — multi-instance: mixed read/write load (4 replicas)"
# shellcheck disable=SC2086
go test $GT_FLAGS -tags integration \
    ./internal/handler/... \
    -run "TestMultiInstance_MixedReadWriteLoad"\
    -timeout 5m -v && ok "MixedReadWriteLoad (4 × 100 goroutines)"|| fail "MixedReadWriteLoad"section "D6 — multi-instance: retrieve cross-replica"
# shellcheck disable=SC2086
go test $GT_FLAGS -tags integration \
    ./internal/handler/... \
    -run "TestMultiInstance_CrossInstanceRetrieve"\
    -timeout 2m -v && ok "CrossInstanceRetrieve"|| fail "CrossInstanceRetrieve"section "D7 — multi-instance: consistent stats across all replicas"
# shellcheck disable=SC2086
go test $GT_FLAGS -tags integration \
    ./internal/handler/... \
    -run "TestMultiInstance_StatsConsistencyAcrossInstances"\
    -timeout 2m -v && ok "StatsConsistency"|| fail "StatsConsistency"section "D8 — multi-instance: high-volume (6 replicas × 300 goroutines)"
