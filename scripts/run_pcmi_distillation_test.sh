...
cd "$ROOT"# Same .env used by docker compose (OPENAI_API_KEY, EVENT_BACKEND, ...)
if [[ -f "${ROOT}/.env"]]; then
  set -a
...
PCMI_REDIS_URL="${PCMI_REDIS_URL:-redis://localhost:6379/0}"# Dedicated SOC test tenant: created with forced UUID via direct SQL,
# then an admin API key is generated for it via /v1/admin/api-keys.
PRESET="${PRESET:-soc}"
TENANT_ID="${TENANT_ID:-a1b2c3d4-e5f6-7890-abcd-ef1234567890}"
TENANT_SLUG="${TENANT_SLUG:-}"NUM_INCIDENTS=1000
SEED=42
USE_LLM=0
CUSTOM_DOMAIN=""
SKIP_BUILD=0
...
API_LOG="${OUTPUT_DIR}/api.log"VENV="${ROOT}/.venv_e2e"# --- ANSI / logging ----------------------------------------------------------
...
publish_memory_event() {
  local payload="$1"if [[ "${EVENT_BACKEND}"== "pubsub"]]; then
...
flush_event_stream_backlog() {
  ...
  log "→ DEL pcmi:events (removes memory.stored backlog from ingest)""${COMPOSE[@]}"exec -T redis redis-cli DEL pcmi:events >/dev/null 2>&1 || true
}

# Print metric row: start x, expected z, (optional) current value y
metric_row() {
  local label="$1"x="$2"z="$3"y="${4:-—}"local mark=""if [[ "$y"!= "—"&& "$y"== "$z"]]; then mark="${GREEN}${NC}"elif [[ "$y"!= "—"&& "$y"!= "$z"]]; then mark="${RED}${NC}"fi
  printf "${mark} %-28s  x=%-6s  →  y=%-6s  (expected z=%s)\n""$label""$x""$y""$z"
}

# Verify y == z; increment TEST_FAILED if different
assert_eq() {
  local label="$1"y="$2"z="$3"hint="${4:-}"if [[ "$y"== "$z"]]; then
    ok "${label}: y=${y} == z=${z} (OK)"return 0
  fi
  err "${label}: y=${y} ≠ z=${z} (FAIL)${hint:+ — $hint}"TEST_FAILED=1
  return 1
}

print_test_plan() {
  local x_active="$1"x_distilled="$2"local z_active=$(( x_active + NUM_INCIDENTS ))
  local z_distilled=$(( x_distilled + NUM_REFINE_EVENTS ))
  echo
  echo -e "${CYAN}══════════════════════════════════════════════════════════════════════${NC}"echo -e "${CYAN}  TEST PLAN — each row: start x → final value y, expected z${NC}"echo -e "${CYAN}══════════════════════════════════════════════════════════════════════${NC}"printf "%-28s  x=%-6s  →  y=???     (expected z=%s)\n""Raw memories (active_memories)""$x_active""$z_active"printf "%-28s  x=%-6s  →  y=???     (expected z=%s)\n""Distilled records (distilled_count)""$x_distilled""$z_distilled"printf "%-28s  x=%-6s  →  y=???     (expected z=%s)\n""Redis refine events published""0""$NUM_REFINE_EVENTS"printf "%-28s  x=%-6s  →  y=???     (expected z=%s)\n""Synthetic records generated""0""$NUM_INCIDENTS"if [[ "$STOP_WORKER_DURING_INGEST"-eq 1 ]]; then
    printf "%-28s  x=%-6s  →  y=???     (expected z=%s)\n""memory.stored events consumed""0""0"echo "(worker stopped during ingest: stores do not trigger automatic distillation)"fi
  echo -e "${CYAN}──────────────────────────────────────────────────────────────────────${NC}"echo "Redis events we will send: ${NUM_REFINE_EVENTS}× memory.refine.requested"echo "(one per shard: shard_000..shard_$(printf "%03d"$((NUM_REFINE_EVENTS-1))), ${SHARD_SIZE} records/shard)"echo
}
...
while [[ $# -gt 0 ]]; do
  case "$1"in
    ...
    --num)          NUM_INCIDENTS="${2:?missing N}"NUM_REFINE_EVENTS=$(( (NUM_INCIDENTS + SHARD_SIZE - 1) / SHARD_SIZE ))
                    shift 2 ;;
    --seed)         SEED="${2:?missing seed}"; shift 2 ;;
    --throttle-ms)  THROTTLE_MS="${2:?missing ms}"; shift 2 ;;
    --batch-size)   BATCH_SIZE="${2:?missing N}"; shift 2 ;;
    --distill-batch-size)
                    DISTILL_BATCH_SIZE="${2:?missing N}"SHARD_SIZE="$DISTILL_BATCH_SIZE"NUM_REFINE_EVENTS=$(( (NUM_INCIDENTS + SHARD_SIZE - 1) / SHARD_SIZE ))
                    shift 2 ;;
...
mkdir -p "$OUTPUT_DIR"# --- Preset defaults (path + tenant slug) ------------------------------------
_apply_preset_defaults() {
  case "$PRESET"in
    soc)
      PATH_PREFIX="${PATH_PREFIX:-root.security.incidents.soc}"TENANT_SLUG="${TENANT_SLUG:-soc-test}";;
    finance)
      PATH_PREFIX="${PATH_PREFIX:-root.finance.events}"TENANT_SLUG="${TENANT_SLUG:-finance-test}";;
    advertising)
      PATH_PREFIX="${PATH_PREFIX:-root.marketing.ads}"TENANT_SLUG="${TENANT_SLUG:-ads-test}";;
    healthcare)
      PATH_PREFIX="${PATH_PREFIX:-root.healthcare.ops}"TENANT_SLUG="${TENANT_SLUG:-health-test}";;
    custom)
      PATH_PREFIX="${PATH_PREFIX:-root.custom.synthetic}"TENANT_SLUG="${TENANT_SLUG:-custom-test}"USE_LLM=1
      ;;
    *)
      err "Unknown --preset ${PRESET} (soc|finance|advertising|healthcare|custom)"exit 2
      ;;
  esac
...
if [[ "$PRESET"== "custom"&& -z "$CUSTOM_DOMAIN"]]; then
  err "preset=custom requires --domain \"...\"(used with --llm)"exit 2
fi
...
if ! docker compose version >/dev/null 2>&1; then
  if command -v docker-compose >/dev/null 2>&1; then
    ...
    err "Need 'docker compose' or 'docker-compose' in PATH"exit 1
  fi
fi
...
cleanup() {
  ...
  if [[ "$SKIP_TEARDOWN"-eq 1 ]]; then
    warn "--no-teardown: containers left running."return
  fi
  log "Tearing down docker compose stack…""${COMPOSE[@]}"logs --no-color worker > "$WORKER_LOG"2>&1 || true
...
# 1) Bootstrap infra
# =============================================================================
log "Step 1/6 — Bootstrap infrastructure (postgres + redis + api + worker)"if [[ "$SKIP_BUILD"-eq 0 ]]; then
  log "docker compose build (api + worker)…""${COMPOSE[@]}"build >/dev/null
fi
...
# 2) Health wait
# =============================================================================
log "Step 2/6 — Waiting for healthcheck (postgres, redis, api)"wait_url() {
  local url="$1"tries="${2:-60}"name="${3:-endpoint}"for i in $(seq 1 "$tries"); do
    if curl -fsS -m 2 -H "X-API-Key: ${PCMI_API_KEY}""$url">/dev/null 2>&1; then
      ok "$name healthy ($url) after ${i}s"return 0
    fi
    sleep 1
  done
  err "Timeout waiting for $name at $url"return 1
}
...
wait_url "${PCMI_BASE_URL}/v1/health"90 "PCMI API"# Sanity: worker process up (its container is running)
...
# =============================================================================
log "Step 3.5/6 — Provisioning tenant ${TENANT_ID} + dedicated API key"# (a) create the tenant with forced UUID via direct SQL on compose postgres,
...
SQL
ok "tenant ${TENANT_SLUG} (${TENANT_ID}) present"# (b) create (or reuse) an admin API key for that tenant.
...
EXISTING_KEY_ID="$(
  ...
  log "found existing API key (id=${EXISTING_KEY_ID}) → rotate to get plaintext back"ROTATE_JSON="$(curl -fsS -X POST -H "X-API-Key: ${ADMIN_API_KEY}"-H "Content-Type: application/json"\
    -d "$(jq -nc --arg n "$KEY_NAME"'{name:$n}')"\
    "${PCMI_BASE_URL}/v1/admin/api-keys/${EXISTING_KEY_ID}/rotate")"PCMI_API_KEY="$(echo "$ROTATE_JSON"| jq -r '.api_key')"
else
  log "creating admin API key for tenant"CREATE_JSON="$(curl -fsS -X POST -H "X-API-Key: ${ADMIN_API_KEY}"-H "Content-Type: application/json"\
    -d "$(jq -nc --arg t "$TENANT_ID"--arg n "$KEY_NAME"'{tenant_id:$t, name:$n, role:"admin"}')"\
    "${PCMI_BASE_URL}/v1/admin/api-keys")"PCMI_API_KEY="$(echo "$CREATE_JSON"| jq -r '.api_key')"
fi

if [[ -z "$PCMI_API_KEY"|| "$PCMI_API_KEY"== "null"]]; then
  err "Unable to obtain API key for ${TENANT_ID}"exit 1
fi
...
# =============================================================================
log "Step 5/6 — Ingest ${NUM_INCIDENTS} records (preset=${PRESET}, seed=${SEED}; expected active_memories: x=${X_ACTIVE} → z=${EXPECT_ACTIVE})"# --- 5a) Stop worker to avoid distillation cascade on every store ---
if [[ "$STOP_WORKER_DURING_INGEST"-eq 1 ]]; then
  log "→ stop worker: ${NUM_INCIDENTS} HTTP stores without consuming memory.stored events on Redis""${COMPOSE[@]}"stop worker >/dev/null
fi

# --- 5b) Ingest puro (no redis publish lato Python) -------------------------
SYNTH_ARGS=(
  generate
  --preset "$PRESET"--num "$NUM_INCIDENTS"--seed "$SEED"--tenant-id "$TENANT_ID"--api-url "$PCMI_BASE_URL"--api-key "$PCMI_API_KEY"--output "$JSONL_OUT"--path-prefix "$PATH_PREFIX"--batch-size "$BATCH_SIZE"--throttle-ms "$THROTTLE_MS"--shard-size "$SHARD_SIZE"
)
if [[ "$USE_LLM"-eq 1 ]]; then
  SYNTH_ARGS+=(--llm)
  [[ -n "$CUSTOM_DOMAIN"]] && SYNTH_ARGS+=(--domain "$CUSTOM_DOMAIN")
fi
PYTHONPATH="${ROOT}/scripts"python3 -m pcmi_synth "${SYNTH_ARGS[@]}"ok "Generator completed — JSONL backup: $JSONL_OUT"# --- Immediate ingest check: active_memories x → y, expected z ---
...
for ((sh=0; sh<NUM_REFINE_EVENTS; sh++)); do
  PREFIX_SH="$(printf "%s.shard_%03d""$PATH_PREFIX""$sh")"PAYLOAD_SH="$(jq -nc \
    --arg t "$TENANT_ID"\
    --arg p "$PREFIX_SH"\
    --arg r "shard_$(printf "%03d""$sh")"\
    '{Type:"memory.refine.requested", Payload:{tenant_id:$t, path_prefix:$p, reason:$r}}')"publish_memory_event "$PAYLOAD_SH"REFINE_PUBLISHED=$((REFINE_PUBLISHED + 1))
  if (( REFINE_PUBLISHED % 10 == 0 )); then
    log "refine published: ${REFINE_PUBLISHED}/${NUM_REFINE_EVENTS}"fi
  sleep 1.2
done
...
if [[ "$SKIP_DISTILL"-eq 1 ]]; then
  warn "--skip-distill: skipping distilled_count check."Y_DISTILLED="$X_DISTILLED"
else
  log "Step 6/6 — Waiting for distillation (x=${X_DISTILLED} → z=${EXPECT_DISTILLED}, ${NUM_REFINE_EVENTS} LLM jobs in parallel)"Y_DISTILLED="$X_DISTILLED"for i in $(seq 1 40); do
    Y_DISTILLED="$(fetch_distilled_count)"if (( Y_DISTILLED >= EXPECT_DISTILLED )); then
      ok "distilled_count reached z=${EXPECT_DISTILLED} at poll ${i}/40 (y=${Y_DISTILLED})"break
    fi
    log "poll ${i}/40: distilled_count y=${Y_DISTILLED} (expected z=${EXPECT_DISTILLED})..."sleep 3
  done
  metric_row "distilled_count""$X_DISTILLED""$EXPECT_DISTILLED""$Y_DISTILLED"assert_eq "distilled_count after refine""$Y_DISTILLED""$EXPECT_DISTILLED"\
...
assert_eq "worker 429 hits""$RATE_LIMIT_HITS""$EXPECT_RATE_LIMIT"# Sample delle distilled
echo
...
| jq -r '
    ...
    insights: \((.insights // []) | if type == "array"then map(tostring) | join("| ") else tostring end)"' 2>/dev/null || true
...
| jq -r '
    ...
    | sort_by(.path)
    | .[]
    | "\(.path): \((.source_entry_ids // .sources // []) | length) raw sources"' 2>/dev/null || true
...
if [[ "$TEST_FAILED"-eq 0 ]]; then
  ok "TEST PASS — all metrics: y == z"echo "active_memories: ${X_ACTIVE} → ${Y_ACTIVE} (z=${EXPECT_ACTIVE})"echo "distilled_count: ${X_DISTILLED} → ${Y_DISTILLED} (z=${EXPECT_DISTILLED})"echo "refine events:   0 → ${REFINE_PUBLISHED} (z=${NUM_REFINE_EVENTS})"
else
  err "TEST FAIL — at least one metric has y ≠ z (see  rows above)"exit 1
fi
echo -e "${GREEN}══════════════════════════════════════════════════════════════════════${NC}"
