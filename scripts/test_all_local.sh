...
ENV_BACKUP=".env.pcmi-test-backup"PASS=0
FAIL=0
SKIP=0

section() {
	echo ""echo "╔════════════════════════════════════════════════════════════╗"printf "║ %-58s ║\n""$1"echo "╚════════════════════════════════════════════════════════════╝"
}
...
GOLANGCI_IMAGE="${GOLANGCI_IMAGE:-golangci/golangci-lint:v2.12.2}"run_helm_ci() {
...
run_kubeconform_ci() {
	local manifest="$1"if command -v kubeconform >/dev/null 2>&1; then
		kubeconform -summary -strict -schema-location default "$manifest"return $?
	fi
...
env_set() {
	local key="$1"val="$2"if grep -q "^${key}=".env 2>/dev/null; then
		# macOS/BSD sed
...
restore_env() {
	if [ -f "$ENV_BACKUP"]; then
		cp "$ENV_BACKUP".env
		echo "(restored .env from $ENV_BACKUP)"fi
}
...
if [ -z "$VIOLATIONS"]; then
	...
	bad "unexpected getenv usage:"echo "$VIOLATIONS">&2
fi

section "A2 — build & vet"
go build -o /dev/null ./... && ok "go build"|| bad "go build"
go vet ./... && ok "go vet"|| bad "go vet"section "A4 — unit tests (-race)"
...
go test -race -count=1 $GT_P -timeout "$GT_TIMEOUT"./internal/... ./cmd/... \
	&& ok "race tests"|| bad "race tests"section "A5 — config / .env.example sync"
go test -count=1 ./internal/config/... -run TestEnvExampleStaysInSyncWithConfig \
	&& ok "env.example ↔ config.go"|| bad "env drift"section "A5b — os.Getenv invariant (cmd/, internal/ outside config)"
...
go test -count=1 ./internal/config/... -run TestNoOSGetenvOutsideConfig \
	&& ok "no rogue os.Getenv in production code"|| bad "rogue os.Getenv detected"section "A6 — gRPC ResolveGRPCPort"
go test -count=1 ./internal/grpc/... -run 'TestPortResolutionFromConfig|TestEphemeralListenerWorks' \
	&& ok "gRPC port unit tests"|| bad "gRPC port tests"section "A7 — telemetry unit tests"
go test -count=1 ./internal/telemetry/... && ok "telemetry package tests"|| bad "telemetry tests"section "A8 — golangci-lint (CI job golangci-lint)"
...
if [ "${SKIP_GOVULNCHECK:-}"= "1"]; then
	...
	GOV_TMP="$(mktemp)"set +e
	if curl -sf --max-time 15 https://vuln.go.dev/ >/dev/null 2>&1; then
		go run golang.org/x/vuln/cmd/govulncheck@latest ./... 2>"$GOV_TMP"GOV_EC=$?
	else
		GOV_EC=125
	fi
	set -e
	if [ "$GOV_EC"-eq 0 ]; then
		ok "govulncheck"elif [ "$GOV_EC"-eq 125 ]; then
		skip "govulncheck (offline / vuln.go.dev blocked — CI runs with network)"else
		if [ "${REQUIRE_GOVULNCHECK:-}"= "1"]; then
			bad "govulncheck"sed 's/^/  | /' "$GOV_TMP">&2 || true
		else
			skip "govulncheck failed locally (set REQUIRE_GOVULNCHECK=1 to fail; CI has network)"sed 's/^/  | /' "$GOV_TMP">&2 || true
		fi
	fi
	rm -f "$GOV_TMP"
fi
...
go test -race -count=1 ./internal/deploy/... && ok "internal/deploy tests"|| bad "internal/deploy tests"section "A11 — gRPC TLS tests (PR #3)"
go test -race -count=1 ./internal/grpc/... -run 'TLS|Handshake|BuildServer' && ok "gRPC TLS tests"|| bad "gRPC TLS tests"section "A12 — Helm lint + template + kubeconform (CI job helm-lint)"
if [ "${SKIP_HELM_KUBECONFORM:-}"= "1"]; then
	...
	RENDERED="/tmp/pcmi-ci-rendered-$$-$RANDOM.yaml"if run_helm_ci lint deploy/helm/pcmi --strict; then ok "helm lint --strict"; else bad "helm lint --strict"; fi
	if run_helm_ci template pcmi deploy/helm/pcmi >"$RENDERED"&& [ -s "$RENDERED"]; then
		ok "helm template (default values)"else
		bad "helm template"fi
	if command -v kubeconform >/dev/null 2>&1 || docker info >/dev/null 2>&1; then
		...
		bad "kubeconform: install kubeconform or start Docker ($KUBECONFORM_IMAGE)"fi
	rm -f "$RENDERED"
else
...
go mod verify && ok "go mod verify"|| bad "go mod verify"section "A14 — version constants"
go test -race -count=1 ./internal/version/... && ok "internal/version tests"|| bad "version tests"if [ "${RUN_COVERAGE:-}"= "1"]; then
	section "Coverage gate (optional RUN_COVERAGE=1)"make test-cover && make cover-check && ok "coverage"|| bad "coverage"
fi

if [ "$MODE"= "quick"]; then
	section "SUMMARY (quick)"echo "passed: $PASS  failed: $FAIL  skipped: $SKIP"[ "$FAIL"-eq 0 ] || exit 1
	exit 0
fi

if [ "${SKIP_DOCKER:-}"= "1"]; then
	skip "Docker phases (SKIP_DOCKER=1)"section "SUMMARY"echo "passed: $PASS  failed: $FAIL  skipped: $SKIP"exit 0
fi
...
curl -sf "$WORKER_URL/health"| jq -e '.status == "healthy"' >/dev/null && ok "worker /health"|| bad "worker"section "B3 — store + retrieve (X-API-Key)"
...
go test -race -count=1 -tags=integration ./internal/grpc/... -timeout 5m \
	&& ok "integration grpc tests"|| bad "integration grpc"section "B6 — summarize (OPENAI_API_KEY from .env)"
...
[ "$MAX_ATT"= "5"] && ok "webhook max_attempts=5"|| bad "webhook max_attempts=$MAX_ATT"section "B8 — encryption (PCMI_ENCRYPTION_KEY + encrypt_content)"
...
[ "$PLAIN"= "secret"] && ok "decrypt on retrieve"|| bad "decrypt failed"section "B9 — rate limit from config (admin key → RPM_ADMIN)"
...
if [ -n "$OTEL"]; then
	...
	if $DOCKER_COMPOSE logs api --tail=30 2>&1 | grep -q 'OpenTelemetry: OTLP'; then
		ok "OTLP exporter active in API logs"else
		bad "OTEL vars set but no OTLP log line"fi
else
...
if grep -q '^OPENAI_API_KEY=.\+' .env 2>/dev/null; then
	curl -sS -o /dev/null -X POST "$API_URL/v1/memories"-H "X-API-Key: $KEY"\
		-H "Content-Type: application/json"\
		-d "{\"path\":\"$PATH_BASE.distill\",\"content\":\"distill me\",\"embedding_model\":\"unspecified\"}"sleep 8
	if $DOCKER_COMPOSE logs worker --tail=40 2>&1 | grep -q 'Distillation saved'; then
		ok "distillation after store"else
		skip "distillation not confirmed in logs (may be slow)"fi
else
...
if [ "$WITH_HOST"-eq 1 ]; then
	section "C1 — host go run ./cmd/api (infra-deps only)"$DOCKER_COMPOSE stop api worker 2>/dev/null || true
	make infra-deps-up
	env_set RATE_LIMIT_DISABLED true
	env_set PCMI_ENCRYPTION_KEY "$ENC_KEY"export DATABASE_URL="postgres://pcmi:pcmi@127.0.0.1:5432/pcmi?sslmode=disable"export REDIS_ADDR="127.0.0.1:6379"export API_PORT=8000
	...
	if curl -sf http://127.0.0.1:8000/v1/ready >/dev/null; then
		ok "host API /v1/ready"else
		bad "host API failed — see /tmp/pcmi-api-host.log"tail -20 /tmp/pcmi-api-host.log >&2 || true
	fi
